...
local BloodSprite = Material( "effects/blood_core" )
...
function EFFECT:Init( data )
    ...
    self.PlaybackSpeed = math.Rand( 6, 8 )
    self.Width         = math.Rand( 6, 10 )
    self.ParCount      = 300
...
function EFFECT:Think()
    ...
    if self.ForceDead < CurTime() then
        return false
    end
    ...
    self.Width = self.Width - 1 * FrameSpeed
...
function EFFECT:Render()
    ...
    for k, p in pairs( self.Particles ) do
        ...
        render.DrawSprite( p.Pos, 24, 24, color )

        -- local Sin = math.sin( (pCount))--* math.pi )
