...
SWEP.ViewModelBoneMods = {
    [ "ValveBiped.Grenade_body" ] = { scale = Vector( 0.702, 0.702, 0.702 ), pos = Vector( 0, 0, 0 ), angle = Angle( 0, 0, 0 ) }
}

SWEP.VElements = {
    [ "a" ] = { type = "Model", model = "models/props_lab/jar01b.mdl", bone = "ValveBiped.Grenade_body", rel = "", pos = Vector( 0, 0, 0 ), angle = Angle( 0, 111.039, 180 ), size = Vector( 0.74, 0.74, 0.74 ), color = Color( 255, 255, 255, 255 ), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
    [ "a" ] = { type = "Model", model = "models/props_lab/jar01b.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector( 5, 2, 0 ), angle = Angle( 180, 0, 0 ), size = Vector( 0.755, 0.755, 0.755 ), color = Color( 255, 255, 255, 255 ), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        if GetRoundState() ~= ROUND_ACTIVE then
            self.Owner:PrintMessage( HUD_PRINTTALK, "Wait to use these until the round starts!" )
            return
        end

        if self.Owner:IsCyborg() then
            self.Owner:PrintMessage( HUD_PRINTTALK, "Cyborgs don't need vitamins!" )
            return
        end
    end
    ...
    if SERVER then
        if (self.Owner.Mission == MISSION_NONE) then
            self.Owner:PrintMessage( HUD_PRINTTALK, "You don't need these yet!" )
            self.Owner:PrintMessage( HUD_PRINTTALK, "There are " .. self.TotalVitamins .. " left." )
            return
        end

        local amount = math.Round( math.random( 15, 30 ) )
