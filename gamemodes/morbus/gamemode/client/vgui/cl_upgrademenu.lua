...
--for k,v in player.Iterator() do MsgN(v:Nick()..": "..v.Evo_Points) end
...
function CreateUpgradesMenu()
    ...
    function base:Paint()
        derma.SkinHook( "Paint", "Frame", self, 875, 450 )
        draw.RoundedBox( 8, 0, 0, base:GetWide(), base:GetTall(), Color( 55, 85, 55, 185 ) )
        draw.SimpleTextOutlined( "Upgrade Points: " .. Morbus.Evo_Points, "DefaultLarge", base:GetWide() / 2, 24, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, Color( 0, 0, 0, 255 ) )
    end
...
function UpdateUpgradesMenu()
    ...
    for k, v in pairs( UPGRADES ) do
        ...
        if not up[ v.Tree ].ByTier[ v.Tier ] then up[ v.Tree ].ByTier[ v.Tier ] = {} end

        if not tiers[ v.Tree ] then tiers[ v.Tree ] = {} end

        if not tiers[ v.Tree ][ v.Tier ] then tiers[ v.Tree ][ v.Tier ] = 0 end
    ...
    -- DIVIDERS
    local curx, cury = (UPGRADE_MENU_PADDING * 2), 50
        ...
        for i = 1, mtiers do
            ...
            for j = 1, ups do
                ...
                function obj:OnMousePressed()
                    ...
                    if self.Tier == 3 then
                        if self.Upgrade == UPGRADE.ATTACK_T3 then

                            if (Morbus.Upgrades[ UPGRADE.CLAWS ] or 0) < 3 then
                                LocalPlayer():PrintMessage( 3, "You do not have sufficient upgrades to increase this upgrade." )
                                return
                            end
                        end
