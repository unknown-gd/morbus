...
function AlienScent()
    ...
    if not ply:IsAlien() or not HUD_DEBUG[ 13 ] then return end

    --if Morbus.AlienScent < CurTime() then return end
    local distance = 1800

    if ply:IsBrood() and Morbus.Upgrades[ UPGRADE.SMELLRANGE ] then
        distance = distance + (Morbus.Upgrades[ UPGRADE.SMELLRANGE ] * UPGRADE.SMELLRANGE_AMOUNT)
    end

    for k, ent in player.Iterator() do
        if ent ~= ply then
            if ((ply:GetPos():Distance( ent:GetPos() ) < distance and ent:Alive()) or (ent:IsAlien() and ent:Alive())) and ent:Team() == TEAM_GAME then
                A( ent )
                B( ent )
            end
        end
...
function B( ent )
    ...
    local HP = ent:Health()
    if ent:IsAlien() then
        if LocalPlayer():IsSwarm() or LocalPlayer():GetNWBool( "alienform", false ) then
            HP = Color( 255, 0, 255 )
        else
            HP = Color( 255, 0, 255 )
        end
        ...
        if ent:IsCyborg() then
            HP = Color( 155, 215, 255 )
        else
...
local function RenderScreenspaceEffects()
    if not LocalPlayer():IsAlien() then return end
