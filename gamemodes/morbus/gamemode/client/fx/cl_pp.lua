...
function ScreenEffects()
    ...
    if ply:IsGame() and not ply:GetNWBool("alienform", false) then
        DrawDistortion()
    end

    if not Morbus.Blinded then
        PostProcess()
    else
        Blind()
    end

    if not ply.Fear then return end
...
function PostProcess()
    ...
    DrawColorModify(colorMod)
    DrawBloom(0, 0.5, 2, 2, 1, 1, 1 - (LocalPlayer():Health() / 30), 1 - (LocalPlayer():Health() / 30),
        1 - (LocalPlayer():Health() / 30))
end
