...
function DrawDistortion()
    ...
    if ply:GetBaseSanity() < 700 and ply:IsGame() and not ply:IsSwarm() and not ply:GetNWBool("alienform", false) then
        local mult = 0.3
...
function DrawDistortionAlien()
    ...
    local ply = LocalPlayer()
    if ply:GetNWBool("alienform", false) or ply:IsSwarm() then -- and not GetConVar("morbus_hide_distortion"):GetBool() then
