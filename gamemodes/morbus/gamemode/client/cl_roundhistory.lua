...
function PANEL:Init()
    ...
    self.EXIT.Paint = function()
        ...
        draw.SimpleText("EXIT", "Trebuchet24", self.EXIT:GetWide() / 2, self.EXIT:GetTall() / 2,
        ...
        if (i == 1) then
            for p = 1, #RoundHistory["First"] do
                ...
                self.BARS["Alien" .. p].Icon.Paint = function()
                    draw.RoundedBox(4, 0, 0, self.BARS["Alien" .. p].Icon:GetWide(),
                        self.BARS["Alien" .. p].Icon:GetTall(), self.BARS["Alien" .. p].Icon.BGColor)
                end
                ...
                self.BARS["Alien" .. p].Paint = function()
                    draw.RoundedBox(4, 0, 0, self.BARS["Alien" .. p]:GetWide(), self.BARS["Alien" .. p]:GetTall(),
                        Color(30, 30, 30, 240))
                    draw.RoundedBox(4, 2, 2, self.BARS["Alien" .. p]:GetWide() - 4,
                        self.BARS["Alien" .. p]:GetTall() - 4, self.TBARS[i][3])
                    draw.SimpleTextOutlined(self.TBARS[i][1], "DSMedium", 45, 20, Color(255, 255, 255, 255),
                        TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 255))
                    draw.SimpleTextOutlined(self.TBARS[i][4](RoundHistory[self.TBARS[i][2]], false, p), "MenuLarge",
                        self.BARS["Alien" .. p]:GetWide() - 20, 20, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT,
                        TEXT_ALIGN_CENTER, 1, Color(0, 0, 0, 205))
                end
            end
        else
            ...
            self.BARS[i].Icon.Paint = function()
                draw.RoundedBox(4, 0, 0, self.BARS[i].Icon:GetWide(), self.BARS[i].Icon:GetTall(),
                    self.BARS[i].Icon.BGColor)
            end
            ...
            self.BARS[i].Paint = function()
                ...
                draw.SimpleTextOutlined(self.TBARS[i][1], "DSMedium", 45, 20, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT,
                    TEXT_ALIGN_CENTER, 2, Color(0, 0, 0, 255))
                draw.SimpleTextOutlined(self.TBARS[i][4](RoundHistory[self.TBARS[i][2]]), "MenuLarge",
                    self.BARS[i]:GetWide() - 20, 20, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1,
                    Color(0, 0, 0, 205))
            end
        end
    end
