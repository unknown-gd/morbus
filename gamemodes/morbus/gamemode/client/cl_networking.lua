...
concommand.Add( "mor_debug", function()
    DEBUG_NET = not DEBUG_NET
    MsgN( DEBUG_NET )
end )
...
local function RecieveOOCChat( UM )
    ...
    chat.AddText( Color( 255, 130, 0 ), "[OOC] (", Color( 200, 0, 0 ), realname, Color( 255, 130, 0 ), ") ",
        Color( 255, 255, 0 ), fakename, ":", Color( 255, 255, 255 ), text )
end
...
function RecieveRoundHistory()
    ...
    file.Append( "Morbus/logs/" .. game.GetMap() .. "/" .. "round_log_" .. tostring( os.date( "%m-%d_%H%M" ) ) .. ".txt",
        "#### MORBUS ROUND LOG ####\n" .. os.date() .. "\n" )
    for k, v in pairs( Round_Log ) do
        file.Append(
            "Morbus/logs/" .. game.GetMap() .. "/" .. "round_log_" .. tostring( os.date( "%m-%d_%H%M" ) ) .. ".txt", "[" .. v.
            Time .. "] [" .. v.Type .. "] " .. v.Text .. "\n" )
    end
    ...
    for k, v in player.Iterator() do
        file.Append(
            "Morbus/logs/" .. game.GetMap() .. "/" .. "round_log_" .. tostring( os.date( "%m-%d_%H%M" ) ) .. ".txt",
            "[" .. v:GetFName( true ) .. "] " .. v:SteamID() .. "\n" )
    end
end
