...
function CreateDescriptionBox(obj)
    ...
    xpos, ypos = (xpos + (UPGRADE_SIZE_MENU * 0.85)) + bx,
        (ypos + (UPGRADE_SIZE_MENU * 0.65)) +
        by                                                                                          -- Since Panel:GetPos() refers position relative to parent, I have to add the parent's position to it

    local desc = vgui.Create("DPanel")
    desc:SetPos(xpos - (ScrW() * 0.25) / 2, ypos + UPGRADE_SIZE_MENU / 2)
    desc:SetSize(ScrW() * 0.25, ScrH() * 0.125)
    desc:MakePopup()
    desc:SetKeyboardInputEnabled(false)
    desc:SetFocusTopLevel(true)
    function desc:Paint()
        draw.RoundedBox(0, 0, 0, self:GetWide(), self:GetTall(), Color(0, 25, 0, 235))
    end

    function desc:OnCursorEntered()
        if not pUpgradesMenu then
            self:Remove()
            pDescriptionBox = nil
            return
        end
    ...
    if obj.Tier > 1 then
        ...
        lab:SetFocusTopLevel(true)
        lab:SetText("Requires " ..
        UPGRADE_TIER_REQUIREMENT .. " points in " .. UPGRADE_TREES[obj.Tree] .. " Tier " .. (obj.Tier - 1))
        lab:SizeToContents()
    end
