...
function DrawAmmoScreen()
    ...
    if ply:Alive() then
        ...
        if weapon and weapon:IsValid() and vmodel and vmodel:IsValid() then
            ...
            if (ammo > 0 and weapon:GetClass() ~= "weapon_glowstick" and weapon:GetClass() ~= "weapon_glowstick_special") then
                GunHud = weapon.GunHud or DefaultGunHud
                GunHud.height = 1.5
                GunHud.width = 3
                if GunHud.enabled then
                    local ironsight = vmodel:GetAttachment(tostring(GunHud.attachmentpoint)) or
                    vmodel:GetAttachment("1")
                    if ironsight or weapon:GetClass() == "weapon_mor_medkit" then
                        ...
                        render.DrawQuadEasy(pos, EyeVector() * -1, tonumber(GunHud.width), tonumber(GunHud.height), nil,
                            180)
                        cam.End3D()
                    end
                end
...
function DrawAmmoScreenTexture()
    ...
    if not HUD_DEBUG[6] then return end
    ...
    if ply:Alive() then
        ...
        if weapon and weapon:IsValid() and vmodel and vmodel:IsValid() then
            ...
            if (ammo > 0 and weapon:GetClass() ~= "weapon_glowstick" and weapon:GetClass() ~= "weapon_glowstick_special") then
                ...
                --Screen noise
                surface.SetDrawColor(math.floor(ScreenColor.r * bgscale), math.floor(ScreenColor.g * bgscale),
                    math.floor(ScreenColor.b * bgscale), 64)
                surface.SetTexture(NoiseTexture)
                surface.DrawTexturedRect(0, 2, TEX_WIDTH, TEX_HEIGHT - 5)

                --Scanlines
                surface.SetDrawColor(math.floor(ScreenColor.r * bgscale), math.floor(ScreenColor.g * bgscale),
                    math.floor(ScreenColor.b * bgscale), 250)
