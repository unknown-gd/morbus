...
function GM:HUDDrawTargetID()
    ...
    if trace.Entity:GetClass() == "prop_ragdoll" then
        ...
        if ent:GetNWBool("HumanBody", false) and distance < 140 then
    ...
    if trace.Entity:GetActiveWeapon() ~= NULL then
        if LocalPlayer():IsAlien() or (trace.Entity:GetNWBool("alienform", false) ~= true and trace.Entity:GetActiveWeapon():GetClass() ~= "weapon_mor_swarm" and not trace.Entity:GetNWBool("cloaked", false)) then
            local ent = trace.Entity
            text = ent:GetFName()
            local clr = 255

            if ent:IsAlien() and LocalPlayer():IsAlien() or ent:IsSwarm() then
