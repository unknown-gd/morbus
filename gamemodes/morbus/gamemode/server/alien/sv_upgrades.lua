...
function Increase_Upgrade( ply, cmd, args )
    ...
    if not ply:IsBrood() then return end
    ...
    if (ply:GetTierPoints( up.Tree, up.Tier - 1 ) >= UPGRADE_TIER_REQUIREMENT) or (up.Tier == 1) then
...
function CalcUpgrade()
    return POINTS_PER_KILL + math.Round( player.GetCount() * POINTS_PER_KILL2 )
end
