...
local function RemoveWeaponEntities()
    ...
    for _, sw in pairs( ents.MORBUS.GetSpawnableSWEPs() ) do
        for k, ent in ipairs( ents.FindByClass( sw.ClassName ) ) do
            ent:Remove()
        end
    end
...
function ents.MORBUS.GetArmingFile( map )
    return file.Read( "data_static/maps/" .. map .. "_morbus.txt", "GAME" )
end

function ents.MORBUS.CanImportEntities( map )
    if not tostring( map ) then return false end

    return file.Exists( "data_static/maps/" .. map .. "_morbus.txt", "GAME" )
end
...
local function ImportSettings( map )
    ...
    local fname = "data_static/maps/" .. map .. "_morbus.txt"

    local settings = {}
    if buf == nil then
        print(
            "Warning! No arming file detected! Server Owners should ensure Server has an arming file associated with the current map!" )
        for i, ply in player.Iterator() do
            ply:ChatPrint( "Warning!!!" )
            ply:ChatPrint( "No arming file detected!!!" )
            ply:ChatPrint(
                "Players should request Server Admin to ensure Server has an arming file associated with the current map in the maps folder!" )
        end
...
local classremap = {
    ...
    weapon_mor_m20 = "weapon_mor_tech_rifle",
    weapon_glowstick_normal = "weapon_glowstick",
    weapon_glowstick_flying = "weapon_glowstick_grav"
}
