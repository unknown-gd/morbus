...
function NeededRTV()
    local need = math.ceil( player.GetCount() * 0.51 )
    return (need - RTV_COUNT)
end

function RTV( ply )
    if ply.RTV or ply.RTV == true then return end

    if not rtvAllowed then return end
...
function SMV.CreateMapList()
    ...
    if playerCount > 24 then
        ...
        table.insert( SMV.ExcludedMaps, "mor_halcyon" )
    end

    if playerCount < 10 then
        table.insert( SMV.ExcludedMaps, "mor_isolation_cv1" )
        if playerCount < 12 then
            table.insert( SMV.ExcludedMaps, "mor_turbito" )
            if playerCount < 14 then
    ...
    for k, v in pairs( SMV.Maps ) do
        if not table.KeyFromValue( SMV.ExcludedMaps, v ) then
