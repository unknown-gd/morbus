...
function plymeta:LightReset()
    ...
    self:ResetMission()
    self.Mission_Next = CurTime() +
    math.random(GetConVar("morbus_mission_next_time_min"):GetInt(), GetConVar("morbus_mission_next_time_max"):GetInt())
    self.Moving = false
    self.Moved = false
    self.Cloaked = 0
    ...
    -- For right click upgrades
    self.T3Attack = 0
    self.T3AttackCoolDown = CurTime()
    self.T3Defense = 0
    self.T3DefenseCoolDown = CurTime()
    self.NextScreech = CurTime()

    self:SetSwarmPoints(0)
...
function plymeta:SetSpeed()
    ...
    if self:IsHuman() or self:IsBrood() then
        self:CalcWeight()
    end
end
...
function plymeta:DeathSound() -- TO DO
    ...
    elseif self:GetGender() == GENDER_FEMALE then
        sound.Play(table.Random(Sounds.Female.Death), self:GetPos(), 300, 100)
    elseif self:GetGender() == GENDER_MALE then
        sound.Play(table.Random(Sounds.Male.Death), self:GetPos(), 300, 100)
    elseif self:GetGender() == GENDER_CYBORG then
        sound.Play(table.Random(Sounds.Cyborg.Death), self:GetPos(), 300, 100)
    end
end
...
function plymeta:KilledAlien()
    ...
    elseif self:GetGender() == GENDER_CYBORG then
        self:EmitSound(table.Random(Sounds.Cyborg.KillAlien), 100, 100)
    end
end
...
function plymeta:SelectName()
    ...
    if gen == GENDER_CYBORG then
        name = table.Random(NAMES.CYBORGFIRST) .. " " .. table.Random(NAMES.CYBORGLAST)
    end
...
function plymeta:SelectModel()
    ...
    elseif (gender == GENDER_CYBORG) then
        self:SetGender(3)
        return table.Random(Models.Cyborg)
    end
end

function plymeta:SetupPlayer()
    self:SetGender(self:SelectGender())
    self:SetNWInt("gender", self:GetGender())
    self:SetNWString("fakename", self:SelectName())
    self:SetNWBool("cloaked", self.Cloaking)
    self.WantedModel = self:SelectModel()
end

function plymeta:Cyborgify()
    SetGlobalInt("total_cyborgs", GetGlobalInt("total_cyborgs") + 1)

    self:SetGender(GENDER_CYBORG)
    self:SetModel(self:SelectModel())
    self:SetNWString("fakename", self:SelectName())

    self:SetTeam(TEAM_GAME)

    GAMEMODE:SetPlayerSpeed(self, CYBORG_SPEED, CYBORG_SPEED)
    self:SetHealth(100)

    self:SetRole(ROLE_CYBORG)
    self.Mission = MISSION_PURGE
    self:SendMission()
...
function plymeta:SpawnForRound(dead)
    ...
    if not self:ShouldReset() then return end
...
function plymeta:GetTierPoints(tree, tier)
    ...
    for k, v in pairs(UPGRADES) do
        if (v.Tier == tier) and (v.Tree == tree) then
