...
function GM:PlayerInitialSpawn( ply )
    ...
    ply.FreeKill = 0
    ply.T3Attack = 0
    ply.T3AttackCoolDown = CurTime()
    ply.T3Defense = 0
    ply.T3DefenseCoolDown = CurTime()
    ply.DamageFactor = 1
    ply:InitSanity()
    ...
    if file.Exists( "Morbus/" .. sid .. ".txt", "DATA" ) then
        local content = file.Read( "Morbus/" .. sid .. ".txt" )
        if content ~= nil then
            local segs = string.Explode( "|", content )
            if tostring( segs[ 1 ] ) ~= "" then
                ply.ForceName = segs[ 1 ]
            end

            if segs[ 2 ] ~= "" then
                ply.ForceGender = tonumber( segs[ 2 ] )
            end

            if tonumber( segs[ 3 ] ) > 0 then
                ply:SetNWInt( "Donator", tonumber( segs[ 3 ] ) )
            end
        end
    end
    ...
    if rstate <= ROUND_PREP then
        ...
        if Swarm_Respawns > 0 or rstate == ROUND_EVAC then
...
function Force_Player( ply )
    if (ply:SteamID() == "STEAM_0:0:20749231") then --PLease leave [Remscar Original Morbus Dev]
        ply.ForceName =
        "Zachary Nawar"                             --This is me the Dev, this just gives me my name and Red scoreboard name color
        ply.ForceGender = 2
...
function GM:PlayerSpawn( ply )
    if (GetRoundState() == ROUND_ACTIVE) and (ply.NextSpawnTime and ply.NextSpawnTime > CurTime()) then return end
    ...
    if GAMEMODE.Nightmare and ply:IsGame() and not ply:IsSwarm() then
        if not ply:HasWeapon( "weapon_glowstick_special" ) then
...
function GM:OnPlayerHitGround( ply, in_water, on_floater, speed )
    ...
    if math.floor( damage ) > 0 then
        ...
        if ply:GetNWBool( "alienform", false ) == true or ply:IsSwarm() then
            if ply.Upgrades[ UPGRADE.JUMP ] or ply:IsSwarm() then
...
function GM:IsSpawnpointSuitable( ply, spwn, force, rigged )
    if not IsValid( ply ) or not ply:IsGame() then return true end
    ...
    for k, p in pairs( blocking ) do
        if IsValid( p ) and p:IsPlayer() and p:IsGame() and p:Alive() and p ~= ply then
            if force then
                p:Kill()
            else
                return false
...
function GM:PlayerSelectSpawn( ply )
    ...
    if not ply:IsSwarm() and not ply:IsBrood() then
        ...
        if (not self.ASpawnPoints) or (table.Count( self.ASpawnPoints ) == 0) or (not IsTableOfEntitiesValid( self.ASpawnPoints )) then
            local tbl = {}
            for _, e in ipairs( ents.FindByClass( "info_alien_spawn" ) ) do
                if IsValid( e ) and (not e.BeingRemoved) then
                    table.insert( tbl, e )
                end
            end
        ...
        if num == 0 then
            Error( "Warning! No spawn entity found! This may suggest the Armer never placed a swarm spawn entity! \n" )
            return
        end
