...
function plymeta:GetCyborg() return self:GetRole() == ROLE_CYBORG end

function plymeta:GetAlien()
    if (self:GetRole() == ROLE_SWARM or self:GetRole() == ROLE_BROOD or self:Team() == TEAM_SPEC) then
        return true
    else
        return false
    end
...
plymeta.IsCyborg = plymeta.GetCyborg
...
function plymeta:IsActiveCyborg() return self:IsActiveRole( ROLE_CYBORG ) end
...
function plymeta:GetFName( fake )
    if not IsValid( self ) then return "disconnected" end
